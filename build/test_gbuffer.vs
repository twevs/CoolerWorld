...
void main()
{
    ...
    fragPosVS = vec3(viewMatrix * modelMatrix * vec4(aPos, 1.f));
    texCoords = aTexCoords;
	
	// NOTE: Assimp's aiProcess_CalcTangentSpace produces unit vectors.
	vec3 tangent = normalize(vec3(viewMatrix * vec4(normalMatrix * aTangent, 0.f)));
	vec3 norm = normalize(vec3(viewMatrix * vec4(normalMatrix * aNormal, 0.f)));
